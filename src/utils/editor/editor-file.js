...
class EditorFile {
	...
	setContent(content) {
		...
		content = content.replace(this.regImg, function(str) {
			...
			str = str.replace(/src\=\"[^"]+\"/gim, function(str1) {
				// 添加全路径
				str1 = 'src="' + sessionStorage.FileServer + str1.split('"')[1] + '"';
				return str1;
			})
			return str;
		});
		...
		content = content.replace(this.regAudioPath, function(str) {
			...
			str = str.replace(/url\=\"[^"]+\"/gim, function(str1) {
				// 交换值
				str1 = 'src="' + str1.split('"')[1] + '" ' + 'url="' + sessionStorage.FileServer + tmpSrc.split('"')[1] + '"';
				return str1;
			})
	...
	addServerToPath(content) {
		...
		//找到所有type="uploadImg"的img标签，对内容进行修改
		content = content.replace(this.regImg, function(str) {
			console.log(sessionStorage.FileServer, str)
			// 提取url
			str = str.replace(/src\=\"[^"]+\"/gim, function(str1) {
				// 添加全路径
				str1 = 'src="' + sessionStorage.FileServer + str1.split('"')[1] + "?" + window.NormalImgThumb + '"';
		...
		content = content.replace(this.regAudioPath, function(str) {
			...
			str = str.replace(/src\=\"[^"]+\"/gim, function(str1) {
				// 添加全路径
				str1 = 'src="' + sessionStorage.FileServer + str1.split('"')[1].split('.')[0] + '.mp3"' + " controls ";
				return str1;
			})
			return str;
		});
		...
		content = content.replace(this.regVideoPath, function(str) {
			...
			str = str.replace(/src\=\"[^"]+\"/gim, function(str1) {
				...
				str1 = 'src="' + sessionStorage.FileServer + str1.split('"')[1].split('.')[0] + '.mp4"' + " controls preload='meta' ";
				return str1;
			})
			...
			str = str.replace(/poster\=\"[^"]+\"/gim, function(str1) {
				...
				str1 = 'poster="' + sessionStorage.FileServer + str1.split('"')[1] + '"';
				return str1;
			})
			return str;
		});
		...
		content = content.replace(this.regAttachPath, function(str) {
			...
			str = str.replace(/href\=\"[^"]+\"/gim, function(str1) {
				// 添加全路径
				str1 = 'href="' + sessionStorage.FileServer + str1.split('"')[1] + '"';
				return str1;
			})
			return str;
		});
	...
	imgJsonToHtml(imgArray) {
		...
		imgArray.forEach((item, i) => {
			...
			tpl = tpl + "<img src='" + sessionStorage.FileServer + item.uuid + fileType + "' type='uploadImg'  path='" + item.save_path + '" name="' + item.save_file + "'/>"
		})
